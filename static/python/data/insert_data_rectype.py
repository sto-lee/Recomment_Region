...
def insert_data_rectype():
    # CSV 파일 경로 설정
    DATA_PATH = "static/data/dataset/final_csv/"
    csv_file_path = os.path.join(DATA_PATH, "RecType.csv")
    ...
    try:
        with open(csv_file_path, mode="r", encoding='cp949') as file:
            ...
            # INSERT 쿼리 생성
            query = f"INSERT INTO {table_name} (type_name, type_desc) VALUES (%s, %s)"
            
            # 데이터 삽입
            for row in csv_reader:
                mapped_row = (int(row[0]), int(row[1]))  # 순서 매핑
